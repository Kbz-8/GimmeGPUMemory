...
namespace Ggm
{
	...
	void DeviceAllocator::AttachToDevice(VkDevice device, VkPhysicalDevice physical, VulkanFunctions functions) noexcept
	{
		assert(device != VK_NULL_HANDLE && "Invalid logical device");
		assert(physical != VK_NULL_HANDLE && "Invalid physical device");

		m_device = device;
		m_physical = physical;
		m_fns = functions;

		m_fns.vkGetPhysicalDeviceMemoryProperties(physical, &m_mem_props);
	}
	...
	{
		assert(m_device != VK_NULL_HANDLE && "Invalid logical device");
		assert(m_physical != VK_NULL_HANDLE && "Invalid physical device");
		...
		m_chunks.emplace_back(std::make_unique<MemoryChunk>(m_device, m_physical, m_fns, chunk_size, memory_type_index, dedicated_chunk, m_vram_usage, m_vram_host_visible_usage, m_last_chunk_creation_failed));
		...
		if(m_last_chunk_creation_failed && !dedicated_chunk)
		{
			...
			while(m_last_chunk_creation_failed && new_block_size_shift < NEW_BLOCK_SIZE_SHIFT_MAX)
			{
				...
				m_chunks.emplace_back(std::make_unique<MemoryChunk>(m_device, m_physical, m_fns, chunk_size, memory_type_index, false, m_vram_usage, m_vram_host_visible_usage, m_last_chunk_creation_failed));
			}
		}
		...
		{
			std::cerr << "Device Allocator: could not allocate a memory chunk" << std::endl;
			std::abort();
		}
	...
	void DeviceAllocator::Deallocate(const MemoryBlock& block)
	{
		...
		std::cerr << "Device Allocator: unable to free a block; could not find it's chunk" << std::endl;
	}
