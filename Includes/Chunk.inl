...
namespace Ggm
{
	MemoryChunk::MemoryChunk(VkDevice device, VkPhysicalDevice physical, VulkanFunctions fns, VkDeviceSize size, std::int32_t memory_type_index, bool is_dedicated, std::uint32_t& vram_usage, std::uint32_t& vram_host_visible_usage, bool& alloc_failed)
		: m_fns(fns), m_device(device), m_physical(physical), m_size(size), m_memory_type_index(memory_type_index), m_is_dedicated(is_dedicated)
	{
		...
		if(m_fns.vkAllocateMemory(m_device, &alloc_info, nullptr, &m_memory) != VK_SUCCESS)
		{
			alloc_failed = true;
			return;
		}
		...
		m_fns.vkGetPhysicalDeviceMemoryProperties(m_physical, &properties);
		...
		{
			if(m_fns.vkMapMemory(m_device, m_memory, 0, VK_WHOLE_SIZE, 0, &p_map) != VK_SUCCESS)
			{
				std::cerr << "Vulkan: failed to map a host visible chunk" << std::endl;
				std::abort();
			}
	...
	void MemoryChunk::Deallocate(const MemoryBlock& block)
	{
		...
		if(it == m_blocks.end())
		{
			std::cerr << "Memory Chunk : cannot deallocate a block that is owned by another chunk" << std::endl;
			std::abort();
		}
		it->free = true;
	...
	{
		m_fns.vkFreeMemory(m_device, m_memory, nullptr);
	}
}
